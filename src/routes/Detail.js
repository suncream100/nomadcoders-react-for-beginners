...
function Detail() {
  ...
  useEffect(() => {
    const getMovie = async () => {
      const json = await (
        await fetch(
          `https://api.themoviedb.org/3/movie/${id}?api_key=5984953a74d52b49b9aa45ad23721fa5`
        )
      ).json();
      setDetail(json);
      setLoading(false);
    ...
    const getCredit = async () => {
      const json = await (
        await fetch(
          `https://api.themoviedb.org/3/movie/${id}/credits?api_key=5984953a74d52b49b9aa45ad23721fa5`
        )
      ).json();
    ...
    <div className="container">
      <div className="inner_container">
        <div className={styles.wrap_detail}>
          <div
            className={styles.bg_detail}
            style={
              detail.production_countries !== undefined
                ? {
                    backgroundImage: `url(https://image.tmdb.org/t/p/w780${detail.backdrop_path})`,
                  }
                : null
            }
          ></div>
